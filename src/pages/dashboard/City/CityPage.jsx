...
export const CityPage = () => {
    ...
    const handleDeleteProduct = async(id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const response = await Phone.deleteCity(id)
    ...
    const renderTableData = currentItems.map((item, index) => (
        <tr key={item._id}>
            ...
            <td className="px-4 py-2 border bg-white text-center">{item.idDepartamento.nombre}</td>
            <td className="px-4 py-2 border bg-white text-center">{item.idDepartamento.idPais.nombre}</td>
            <td className="px-4 py-2 border bg-white text-center">
                <div className="flex gap-x-2">
                    ...
                    <Link to={`/dashboard/city/detail/${item._id}`} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        Ver Detalles
                    </Link>
                </div>
            </td>
                ...
                <table className="min-w-full table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="px-4 py-2 border-white border-2 bg-rose-200">Departamento</th>
                            <th className="px-4 py-2 border-white border-2 bg-rose-200">Pais</th>
